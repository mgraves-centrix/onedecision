-- Baseline schema (PostgreSQL). The deployment target.
-- Semantically identical to 0001_baseline.sqlite.sql, with the types the demo
-- backend cannot express: real booleans, exact-decimal money, identity columns,
-- and append-only enforcement in PL/pgSQL.
...
-- Money is NUMERIC, never floating point. This system authorizes spending
-- against a cap; a binary rounding artifact at the boundary is not acceptable.
...
CREATE TABLE IF NOT EXISTS return_cases (
    ...
    new_damage_present  BOOLEAN NOT NULL,
...
-- `created_at` is part of the hashed entry content, so it is stored as the exact
-- string that was hashed rather than a TIMESTAMPTZ the driver would re-render.
-- `recorded_at` is the operational timestamp for querying and retention.
--
-- `prev_hash` is UNIQUE: two entries cannot claim the same predecessor, so a
-- concurrent fork of the chain fails as a constraint violation instead of
...
-- Defense in depth: a trigger stops the application from rewriting history, and
-- this stops any role that is not the table owner from doing so either. In a
-- real deployment the application connects as a non-owner role, which makes the
-- append-only property a privilege, not just a trigger. See docs/database.md.
